...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_selectize("gene", "Select gene", genes, multiple=False),
            ui.input_selectize("groupby", "Group by metadata", metadata, multiple=False),
        ),
        ui.output_plot("umap"),
...
def server(input, output, session):
    ...
    def umap():
        fig, ax = plt.subplots()
        sc.pl.umap(adata, color=input.gene(), ax=ax, show=False)
        return fig
